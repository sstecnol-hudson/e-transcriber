(() => {
  ...
  class CIDMapper {
    ...
    async loadConfig() {
      ...
      try {
        ...
        if (typeof module !== 'undefined' && module.exports && typeof require !== 'undefined') {
          ...
          const res = await fetch('modules/diagnostic-rag/data/cid-map.json');
          if (res.ok) {
    ...
    async mapDiagnosis(diagnosis, clinicalContext = {}) {
      ...
      const normalizedDiag = (diagnosis || '').trim().toLowerCase();
      let bestKey = null;
      let highestScore = 0;
      ...
      if (highestScore < 0.3) {
        for (const key of Object.keys(this.cidConfig)) {
          ...
          const matches = words.filter(w => w.length > 3 && normalizedDiag.includes(w));
          if (matches.length > 0) {
            bestKey = key;
      ...
      if (mapData.variants && mapData.variants.length > 0) {
        const hasSpecificVariant = mapData.variants.some(v => normalizedDiag.includes(v.description.toLowerCase()));
